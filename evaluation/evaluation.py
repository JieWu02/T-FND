...
def save_embedding(x, fname='embedding.tsv'):
    if not x:  # 检查列表是否为空
        print(f"Warning: Empty embedding list for {fname}, skipping save.")
        return
        
    x = [i.cpu().numpy() for i in x]
    
    if len(x) == 0:  # 再次检查转换后的列表
        print(f"Warning: No valid embeddings for {fname}, skipping save.")
        return
...
def save_2D_embedding(x, fname=''):
    ...
    for i, batch in enumerate(x):
        if len(batch) > 0:  # 确保批次不为空
            ...
            print(f"Warning: Empty batch {i} for {fname}, skipping.")
